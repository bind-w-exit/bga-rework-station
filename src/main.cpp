...
U8G2_ST7920_128X64_F_SW_SPI u8g2(U8G2_R2, /* clock= */ 8, /* data= */ 10, /* select= */ 7);
MUIU8G2 mui;
U8G2_Graph graph;
...
muif_t menuItems[] = {
    ...
    MUIF_VARIABLE("BP", &bgaProfile, mui_u8g2_u8_opt_line_wa_mud_pi) // BGA profile selection
};

// Main Form Definition
fds_t formData[] = MUI_FORM(1)
    MUI_STYLE(1) MUI_XYT("ST", 6, 62, "\u00D3")
        MUI_STYLE(0) MUI_XYAT("BP", 32, 10, 10, "Lead-Free Profile (SnAgCu)|Lead-Based Profile (SnPb)|Mixed Alloy Profile (e.g., SnAgCu with Bi)|Custom Profile (Variable Alloy Ratios)|Low-Temperature Profile (SnBiAg)|High-Temperature Profile (SnAgCu with Increased TAL)|Extended Soak Profile (SnPb with Longer Soak Time)|Fine Pitch Profile (SnAgCu with Reduced TAL)|Custom Profile 1")
            MUI_STYLE(1) MUI_XYT("SS", 122, 62, "\u0081") MUI_XY("GH", 12, 64);
